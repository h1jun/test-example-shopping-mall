...
describe('로그인이 된 경우', () => {
  // 로그인 상태와 장바구니 상품에 대한 store mocking이 필요하다.
  const userId = 10;

  beforeEach(() => {
    // 기존 handlers.js 응답 -> use 함수 내에 응답을 기준으로 테스트 실행된다.
    // 테스트가 완료된 후 기존 handle.js의 응답을 바라보도록 설정해야 하지 않을까? => setupTest.js > afterEach에서 reset해주고 있음
    server.use(
  ...
  it('장바구니(담긴 상품 수와 버튼)와 로그아웃 버튼(사용자 이름: "Maria")이 노출된다.', async () => {
    ...
    // promise 기반으로 응답을 기다려야 하기 때문에 getByRole 대신 findByRole을 사용
    expect(
