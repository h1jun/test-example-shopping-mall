...
it('로딩이 완료된 경우 상품 리스트가 제대로 모두 노출된다', async () => {
  ...
  // 1초 동안 50ms마다 요소가 있는지 조회하고 결과를 반환한다. 재시도 시간은 옵션으로 설정 가능
  const productCards = await screen.findAllByTestId('product-card');

  // Assert
  expect(productCards).toHaveLength(PRODUCT_PAGE_LIMIT);

  // 상품 카드의 상품명, 카테고리, 가격, 장바구니 구매 버튼이 렌더링 되는지 확인
...
it('보여줄 상품 리스트가 더 있는 경우 show more 버튼이 노출되며, 버튼을 누르면 상품 리스트를 더 가져온다.', async () => {
  ...
  // showMore 버튼의 노출 여부는 api에서 상품 목록 데이터를 가져와 렌더링 된 후에 알 수 있다.
...
it('보여줄 상품 리스트가 없는 경우 show more 버튼이 노출되지 않는다.', async () => {
  // Arrange, 모킹 데이터 20개보다 많은 수 50으로 limit을 설정
  await render(<ProductList limit={50} />);

  // findBy 쿼리를 사용하여 먼저 첫 페이지에 해당하는 상품 목록이 렌더링되는 것을 기다려야 함
  await screen.findAllByTestId('product-card');

  // getByText는 결과가 없으면 Error를 던지고, queryByText는 결과가 없으면 null을 반환한다.
...
describe('로그인 상태일 경우', () => {
  ...
  it('구매 버튼 클릭시 addCartItem 메서드가 호출되며, "/cart" 경로로 navigate 함수가 호출된다.', async () => {
    ...
    // 이처럼 다른 페이지의 로직(실제 장바구니에 추가되는 로직)을 검증할 수는 없기 때문에 이러한 모킹 작업이 필요
    // - addCartItemFn 함수를 모킹하여 사용
...
describe('로그인이 되어 있지 않은 경우', () => {
  // user store에 대한 모킹을 하지 않으면 기본적으로 비로그인 상태이기 때문에 별도 setup 함수 없이 바로 테스트를 실행할 수 있다.
